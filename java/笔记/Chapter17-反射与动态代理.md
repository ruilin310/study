...
<img src="assets/image-20250612230049905.png" alt="image-20250612230049905" style="zoom:50%;" />

## 1.3 获取字节码文件对象的三种方式

* Class这个类里面的静态方法forName（“全类名”）-->包名+类名 
  * 最常用
* 通过class属性获取  
  * 用作参数传递
* 通过对象获取字节码文件对象
...
==字节码文件（class）和字节码文件对象（clazz）在硬盘中是唯一的==
...
| Constructor<?>[] getDeclaredConstructors()                   | 获得所有的构造（包含public,private...修饰）                  |
| Constructor<T> getConstructor(Class<?>... parameterTypes)    | 获取指定构造（只能public修饰），参数为相应构造方法的参数类型的字节码文件 |
| Constructor<T> getDeclaredConstructor(Class<?>... parameterTypes) | 获取指定构造（包含public,private...修饰），参数为相应构造方法的参数类型的字节码文件 |
...
获取修饰符（modifiers），名字，形参，创建对象

- `int getModifiers()`:获取权限修饰符
- `Parameter[] getParameters()`:获取构造方法所有参数

- ==`T newInstance(Object... initargs)`：根据指定的构造方法创建对象==
  - `setAccessible(boolean flag)`：设置为true,表示==取消访问检查==-->强行访问private
...
| Field getField(String ==name==)     | 返回单个成员变量对象（只能拿public的），参数是变量的名字 |
...
获取访问修饰符(modifiers)，变量名，变量数据类型，变量的值，修改变量的值

| 方法                                | 说明                    |
| ----------------------------------- | ----------------------- |
| void set(Object obj, Object value） | 为obj对象的属性赋值     |
| Object get(Object obj)              | 获取obj对象对应属性的值 |
...
- `Method[] getMethods()`:包括父类中的公共方法
- `Method[] getDeclaredMethods()`:不能获取父类的==任何==方法，但可以获取本类中所有的
...
<img src="assets/image-20250612231918321.png" alt="image-20250612231918321" style="zoom:50%;" />
...
切记一点：代理可以增强或者拦截的方法都在==接口==中，接口需要写在newProxyInstance的第二个参数里。

## 2.3 代码实现：

1. 获取代理的对象
   自定义：  `代理对象 = ProxyUtil.createProxy(对象);`-->ProxyUtil类是自定义的，实际只有下面的方法

   java提供的方法：`Object Proxy.newProxyInstance()`

2. 再调用代理的相应方法
   `代理对象.调用的方法(参数);`

<img src="assets/image-20250612233901457.png" alt="image-20250612233901457" style="zoom: 67%;" />
    ...
    public static Star createProxy(BigStar bigStar){
       /* java.lang.reflect.Proxy类：提供了为对象产生代理对象的方法：
        ...
        Star star = (Star) Proxy.newProxyInstance(
                ProxyUtil.class.getClassLoader(),//当前类的类加载器。参数一：用于指定用哪个类加载器，去加载生成的代理类
...
​	 对add方法进行==增强==，对remove方法进行拦截，对其他方法不拦截也不增强
...
public class MyProxyDemo1 {
    public static void main(String[] args) {
        ...
        //                  如果写了List，那么表示可以代理List接口里面所有的方法，对这些方法可以增强或者拦截
