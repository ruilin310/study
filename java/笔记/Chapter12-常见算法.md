...
1. **插值查找**：当数据分布比较均匀时，用数值比推测索引比

<img src="assets/image-20250603230926202.png" alt="image-20250603230926202" style="zoom:50%;" />

2. **斐波那契查找**：根据黄金分割点来计算mid
...
**细节：**对于表长较大，而==关键字分布又比较均匀==的查找表来说，插值查找算法的平均性能比折半查找要好的多。反之，数组中如果分布非常不均匀，那么插值查找未必是很合适的选择。
...
​        不同形态的二叉查找树如下图所示：
 ...
 ![20180226113852869](assets/20180226113852869.png) 
...
3. 如果数组中有n个数据，总共我们只要==执行n-1轮==的代码就可以
...
==倒序==遍历有序序列，每当要插入的数据小于当前元素时，==每次都要变换位置==
...
3. ==先执行后面==的指针，找出第一个比基准数小的数字
4. ==再执行前面==的指针，找出第一个比基准数大的数字
5. 交换两个指针指向的数字
6. 直到两个指针相遇-->==此时所指向的数据一定小于基准数==：==先从后先前，再从前向后==;反之不行
7. ==将基准数跟指针指向位置的数字交换位置==，称之为：基准数归位。
8. 第一轮结束之后，基准数左边的数字都是比基准数小的，基准数右边的数字都是比基准数大的。
9. 把基准数左边看做一个序列，把基准数右边看做一个序列，按照刚刚的规则==递归==排序-->递归出口：start>end
