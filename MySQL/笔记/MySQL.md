...
<img src="assets/image-20250614105207995.png" alt="image-20250614105207995" style="zoom:67%;" />

<img src="assets/image-20250614105359333.png" alt="image-20250614105359333" style="zoom:67%;" />
...
而所谓二维表，指的是由行和列组成的表（就类似于Excel表格数据，有表头、有列、有行，
...
<img src="assets/image-20250614105054629.png" alt="image-20250614105054629" style="zoom: 67%;" />
...
- DDL: 数据定义语言，用来**定义**数据库对象（数据库、表、字段）
...
- 创建数据库：
  ...
  - 字符集不建议设置为utf8，而是utf8mb4-->支持4个字节
...
[MySQL数据类型](MySQL数据类型.xlsx)

- char(数字)，varchar(数字)，表示字符串的长度
- double(标度，精度)
  - 标度：有效数字个数
  - 精度：小数个数
...
<img src="assets/image-20250614150415636.png" alt="image-20250614150415636" style="zoom: 67%;" />
...
- 批量添加数据：
  ...
  - ``INSERT INTO 表名 VALUES (值1, 值2, ...), (值1, 值2, ...), (值1, 值2, ...);`-->每一个括号为一组，一组内值对应不同的字段

**注意事项**

- 插入数据时，指定的字段顺序需要与值的顺序一一对应
- 字符串和日期类型数据应该包含在==单引号==中
- 插入的数据大小应该在字段的规定范围内
...
- 如果不带[WHERE 条件]，则修改整个表
- DELETE语句不能修改某一字段的值，可以用UPDATE
...
| =                                 | 等于                                                       |
| <> 或 !=                          | 不等于                                                     |
| BETWEEN ==最小值== AND ==最大值== | 在某个范围内（含最小、最大值），先最大，后最小             |
| IN(...)                           | 在in之后的列表中的值，多选一                               |
| LIKE 占位符                       | 模糊匹配（ ==\_== 匹配==单个==字符，% 匹配==任意个==字符） |
| IS NULL                           | 是NULL                                                     |
...
- 分组之后，查询的**字段**一般为==聚合函数==和==分组字段==，查询其他字段无任何意义
...
| 权限                | 说明                             |
| ------------------- | -------------------------------- |
| ALL, ALL PRIVILEGES | 所有权限，**privilege [权限]**   |
| SELECT              | 查询数据                         |
| INSERT              | 插入数据                         |
| UPDATE              | 修改数据                         |
| DELETE              | 删除数据                         |
...
- 查询权限：
  `SHOW GRANTS FOR '用户名'@'主机名';`-->**grant[授予]**

- 授予权限：
  `GRANT 权限列表 ON 数据库名.表名 TO '用户名'@'主机名';`

- 撤销权限：
  `REVOKE 权限列表 ON 数据库名.表名 FROM '用户名'@'主机名';`-->**revoke[撤销]**
...
| SUBSTRING(str, start, len)       | 返回从字符串str的start位置起的len个长度的字符串-->==索引从1开始== |
...
| DATE_ADD(date, INTERVAL expr type) | 返回一个日期/时间值加上一个时间间隔expr后的时间值。expr是值，type是单位->**interval[间隔]** |
...
| 唯一约束                | 保证该字段的所有==数据==都是唯一、不重复的               | UNIQUE      |
| 主键约束                | 主键是一行数据的唯一标识，要求非空且唯一                 | PRIMARY KEY |
| 默认约束                | 保存数据时，如果未指定该字段的值，则采用默认值           | DEFAULT     |
| 检查约束（8.0.1版本后） | 保证字段值满足某一个条件                                 | CHECK       |
| 外键约束                | 用来让两张图的数据之间建立连接，保证数据的一致性和完整性 | FOREIGN KEY |

约束是作用于表中字段上的，可以在==创建表/修改表==的时候添加约束。
...
-- 方式2
ALTER TABLE 子表名 ADD CONSTRAINT 外键名称 FOREIGN KEY (子表中外键字段名) REFERENCES 主表(主表列名);
...
| 行为        | 说明                                                         |
| ----------- | ------------------------------------------------------------ |
| NO ACTION   | 当在==父表==中删除/更新对应记录时，首先检查该记录是否有对应外键，如果有则不允许删除/更新（与RESTRICT一致）-->==默认== |
...
==实现==：建立第三张中间表，==中间表==**至少包含两个外键**，分别关联两方主键

<img src="assets/image-20250918170923540.png" alt="image-20250918170923540" style="zoom:80%;" />

<img src="assets/image-20250919144640585.png" alt="image-20250919144640585" style="zoom:80%;" />
...
<img src="assets/image-20250625143211633.png" alt="image-20250625143211633" style="zoom:50%;" />
...
- **左外连接**：
  ...
  返回==左表的所有行==，并包括右表中匹配的行，如果右表中没有匹配的行，将返回 NULL 值-->以左表为主体，返回左表所有数据以及所关联的右表数据。==当右表中没有匹配的行时，相关列将显示为 NULL==
...
```mysql
SELECT 字段列表a FROM 表A ...
UNION [ALL]
SELECT 字段列表a FROM 表B ...
```
...
- 子查询==外部==的语句可以是 ==INSERT / UPDATE / DELETE / SELECT== 的任何一个
...
**事务范围**：事务通常以`START TRANSACTION`显式开启，以`COMMIT`或`ROLLBACK`显式==结束==。

**`SELECT` 语句是查询操作，它不会受到事务的影响，因为查询操作是自动提交的。即使在事务中，查询操作也会立即执行并返回结果，而不需要等待事务的提交。**
...
-- 手动提交时，一个指令被执行后，不会被提交到数据库，直到执行commit语句才会提交
SET @@AUTOCOMMIT = 0;
...
| 不可重复读 | 一个事务先后读取同==**一条**记录==，但两次读取的数据不同     |
...
- Repeatable Read：在一个事务两次查询之间，即使另一个事务执行并==提交==了修改语句，也不会影响该事务第二次查询的结果
- Serializable：一个事务执行过程中，另一个事务执行语句会阻塞，直到该事务提交后才会执行
...
- 设置事务隔离级别：
  ...
  - SESSION 是会话级别，表示只针对==当前会话==有效，GLOBAL 表示对==所有会话==有效
...
![结构图](assets/MySQL体系结构.png "结构图")
![层级描述](assets/MySQL体系结构层级含义.png "层级描述")



- 存储引擎就是存储数据、建立==索引==、更新/查询数据等技术的实现方式。
- 存储引擎是==基于表==而不是基于库的，所以存储引擎也可以被称为表引擎。
- 默认存储引擎是InnoDB。
...
- xxx.ibd: xxx代表表名，InnoDB 引擎的每张表都会对应这样一个表空间文件，存储该表的==表结构（frm、sdi）、数据和索引。==-->8.0之后，存储在sdi中，融入到表空间文件ibd
...
**InnoDB 逻辑存储结构：**
![InnoDB逻辑存储结构](assets/逻辑存储结构.png "InnoDB逻辑存储结构")
...
- xxx.sdi: 存储表结构信息-->可打开，存储json格式数据
- xxx.MYD: 存储数据
- xxx.MYI: 存储索引

### Memory

Memory 引擎的表数据是存储在内存中的，受硬件问题、断电问题的影响，只能将这些表作为**临时表或缓存**使用。
...
- **InnoDB**: 如果应用对==事务==的完整性有比较高的要求，在并发条件下要求数据的一致性，数据操作除了插入和查询之外，还包含很多的更新、删除操作，则 InnoDB 是比较合适的选择-->只有InnoDB支持事务
...
<img src="assets/image-20250919143457007.png" alt="image-20250919143457007" style="zoom:80%;" />

### explain

EXPLAIN 或者 DESC 命令获取 MySQL 如何执行 SELECT 语句的信息，包括在 SELECT 语句执行过程中表**如何连接和连接的顺序**。
语法：
...
![image-20250919143927339](assets/image-20250919143927339.png)
...
![二叉树](assets/二叉树.png "二叉树")

二叉树的缺点可以用红黑树来解决：
![红黑树](assets/红黑树.png "红黑树")
...
B-Tree (多路平衡查找树) 以一棵最大度数（max-degree，指一个节点的子节点个数）为5（5阶）的 b-tree 为例（每个节点最多存储**4个key，5个指针**）

![B-Tree结构](assets/B-Tree结构.png "B-Tree结构")
...
![B+Tree结构图](assets/B+Tree结构图.png "B+Tree结构图")
...
![MySQL B+Tree 结构图](assets/结构图.png "MySQL B+Tree 结构图")
...
- 无法利用索引完成排序操作
- 查询效率高，通常只需要==一次检索==就可以了，效率通常要高于 B+Tree 索引-->若出现哈希碰撞，则多次检索
...
- 相对于 Hash 索引，B+Tree 00支持范围匹配及排序操作
...
| 唯一索引 | 避免同一个表中某数据==列==中的值重复                 | 可以有多个                   | UNIQUE   |
| 常规索引 | 快速定位特定数据                                     | 可以有多个                   |          |
...
| 分类                              | 含义                                                         | 特点                                  |
| --------------------------------- | ------------------------------------------------------------ | ------------------------------------- |
| 聚集索引(Clustered Index)         | 将数据存储与索引放一块，索引结构的叶子节点保存了==行数据==   | 必须有，而且只有==一个==-->存放行数据 |
| 二级（辅助）索引(Secondary Index) | 将数据与索引分开存储，索引结构的叶子节点关联的是对应的==主键== | 可以存在多个                          |
...
![大致原理](assets/原理图.png "大致原理")

**回表查询**：

![演示图](assets/演示图.png "演示图")
...
- 创建索引：
  ...
  - 同时为多个字段添加索引，为==联合索引==。默认每个字段升序，第一个相等时再下一个排序
  - 创建索引时，可以==指定各字段的排序==，如`create index idx_user_age_phone_ad on tb_user(age asc, phone desc);`
  

<img src="assets/image-20250919165717748.png" alt="image-20250919165717748" style="zoom:80%;" />
...
- 命名规范
  ...
  索引名通常以idx_开头，后跟表名和字段名，如`idx_students_name`
...
1. 如果==索引关联了多列==（联合索引），要遵守最左前缀法则，最左前缀法则指的是查询从索引的最左列开始，并且不跳过索引中的列。-->==语句中索引的顺序不影响==
   如果跳跃某一列，索引将部分失效（后面的字段索引失效）。

2. 联合索引中，出现**范围查询 '>' '<'**，范围查询右侧的列索引失效。可以用>=或者<=来规避索引失效问题。

#### 索引失效情况

1. 在==索引列==上进行运算操作，索引将失效。
...
2. 字符串类型字段使用时，不加引号，索引将失效。
   ...
   - 对于联合索引，该字段所对应的索引列失效，其他不失效

3. 模糊查询中，如果仅仅是尾部模糊匹配，索引不会失效；如果是头部模糊匹配，索引失效。
...
use 是**建议**，实际使用哪个索引 MySQL 还会自己权衡运行速度去更改，force就是无论如何都强制使用该索引。
...
​	如果在辅助（二级）索引中找聚集索引，如`select id, name from xxx where name='xxx';`，也只需要通过辅助索引(name)查找到对应的id，返回name和name索引对应的id即可，只需要一次查询；
...
语法：`create index idx_xxxx on table_name(columnn(n));`-->n是前缀长度
...
1. 针对于数据量较大（大于一百万），且查询比较频繁的表建立索引
2. 针对于常作为查询条件（where）、排序（order by）、分组（group by）操作的字段建立索引
3. 尽量选择区分度高的列作为索引，尽量建立唯一索引，区分度越高，使用索引的效率越高
4. 如果是字符串类型的字段，字段长度较长，可以针对于字段的特点，建立前缀索引
5. 尽量使用联合索引，减少单列索引，查询时，联合索引很多时候可以覆盖索引，节省存储空间，避免回表，提高查询效率
6. 要**控制索引的数量**，索引并不是多多益善，索引越多，维护索引结构的代价就越大，会影响增删改的效率
7. 如果索引列不能存储NULL值，请在**==创建表时使用NOT NULL约束==**它。当优化器知道每列是否包含NULL值时，它可以更好地确定哪个索引最有效地用于查询
...
![image-20250919173852095](assets/image-20250919173852095.png)
...
- **页分裂**
  ...
  - 页可以为空，也可以填充一般，也可以填充100%，每个页包含了2~N行数据（如果一行数据过大，会行溢出），根据主键排列。-->==只包含一行，则为链表==
  - 乱序插入时，若所插入的页已经过了50%，则将后半部分分离出一个新的页，在新的页中将后半部分数据和所插入的数据进行排列

  - 索引组织表根据主键顺序排列，但主键不一定顺序插入，此时，就要用到页分裂


<img src="assets/image-20251023162435838.png" alt="image-20251023162435838" style="zoom:67%;" />

<img src="assets/image-20251023162512489.png" alt="image-20251023162512489" style="zoom:67%;" />
...
![image-20250922143652589](assets/image-20250922143652589.png)



![image-20250922143717540](assets/image-20250922143717540.png)
...
- 尽量不要使用 UUID 做主键或者是其他的自然主键，如身份证号-->乱序，长度高
- 业务操作时，避免对主键的修改
...
- 根据排序字段建立合适的索引，多字段排序时，也遵循最左前缀法则-->==顺序要和索引字段相同==

  创建索引语句：：`create index idx_user_age_phone_ad on tb_user(age asc, phone desc);`

  ![image-20250922150250425](assets/image-20250922150250425.png)
...
如索引为`idx_user_pro_age_stat`，则句式可以是`select ... where profession order by age`，这样也符合最左前缀法则-->==where + group by==
...
当使用`WITH [CASCADED | LOCAL] CHECK OPTION`子句创建视图时，MySOL会通过视图检查正在更改的每个行，例如 插入，更新，删除，以使其符合视图的==定义条件==。MVSOL允许基于另一个视图创建视图，它还会检查依赖视图中的规则以保持一致性。-->不符合条件的语句无法执行

为了确定检查的范围，mysql 提供了两个选项:CASCADED 和 LOCAL，==默认值为CASCADED。==

- **cascaded：**在对创建时含有该字段的视图，插入数据时，==该视图及其所依赖的视图都会加上检查选项==，需要所有视图的==条件(where)==都满足才能够插入成功。

  - **若一个视图有cascaded检查选项，则会给所依赖的所有视图加上cascaded检查选项**

  - 若当前创建的视图无检查选项，而所依赖的视图有，MySQL会从上到下，依次检查。本视图无检查选项，不会检查，但会检查所有依赖的视图（若为cascaded,会为下面每一个视图添加cascaded）


  ![image-20250922165226919](assets/image-20250922165226919.png)

- **local：**在对创建时含有该字段的视图，插入数据时，对于该视图及其所依赖的视图中**含有检查选项的视图**进行检查判断。

  - MySQL会从上到下，依次检查。但local不会传递，若视图无检查选项，则不传递
...
要使视图可更新，视图中的==行==与基础表中的==行==之间**必须存在一对一的关系**。
...
+ **安全**  
  数据库表可以授权，但不能授权到数据库表特定行和特定的列上。通过视图用户只能查询和修改他们所能见到的数据(字段)。
+ **数据独立**  
...
select * from tb_user view;

-- 2.查询每个学生所选修的课程（三张表联查），这个功能在很多的业务中都有使用到，为了简化操作，定义一个视图。
create view tb_stu_course_view as
select s.name student_name, s.num student_num, c.name course_name 
...
<img src="assets/image-20250922172851864.png" alt="image-20250922172851864" style="zoom:80%;" />
...
- 查询数据库的存储过程及状态信息
  ...
  INFORMATION_SCHEMA.ROUTINES是一张表
...
用户定义变量 是用户根据需要自己定义的变量，用户变量不用提前声明，在用的时候直接用“@变量名”使用就可以。其作用域为当前会话(连接)。

**赋值：**

- `SET @var_name = expr [, @var_name = expr]...;`
- ``SET @var_name := expr [, @var_name := expr]...;`
- `SELECT @var_name[, @var name ...];`
...
- **如何返回**：
  - out:==没有return，只要在调用存储过程时传入返回的变量就行==
  - inout:现为变量赋值，在传入变量。既作为参数，又作为返回值
...
游标(CURSOR)是用来==存储查询结果集==的**数据类型**,在存储过程和函数中可以使用游标对结果集进行循环的处理。游标的使用包括游标的声明、OPEN、FETCH和 CLOSE，其语法分别如下。
...
```mysql
-- condition_value发生后，执行handler_action 和 statemant
...
characteristic说明:-- 描述存储函数
· DETERMINISTIC:相同的输入参数总是产生相同的结果
· NO SQL:存储函数不包含 SQL语句。
· READS SOL DATA:存储函数包含读取数据的语句，但不包含写入数据的语句,
```
...
**使用==别名== OLD 和 NEW 来引用触发器中发生变化的记录内容**，这与其他的数据库是相似的。现在触发器还只支持==行级触发==，不支持语句级触发。-->触发器为行级触发器，所以更改几行数据则出发几次

|   触发器类型    | NEW 和 OLD                                             |
| :-------------: | ------------------------------------------------------ |
| insert 型触发器 | NEW 表示将要或者已经新增的数据，无old                  |
...
MDL加锁过程是系统自动控制，无需显式使用，在访问一张表的时候会自动加上。MDL锁主要作用是维护表元数据的数据一致性，在表上有活动事务的时候，不可以对==元数据==进行写入操作（不能DDL）。**为了避免DML与DDL冲突，保证读写的正确性。**-->元数据（==表结构==）

- 对一张表进行 CRUD（增删改查） 操作时，加的是 **MDL 共享锁**；
- 对一张**表结构**进行变更操作的时候，加的是 **MDL 互斥锁**；
...
|              ==alter== table ...              |               EXCLYSIVE               |              与其他的MDL都==互斥==               |
...
- 意向共享锁(IS)：与表锁共享锁兼容，与表锁排他锁互斥
   `select语句 lock in share mode` 添加

- 意向独占锁(IX)：与表锁共享锁和排他锁都互斥。
   `insert、update、delete、select...for update  `自动添加
...
InnoDB的数据是基于索引组织的，行锁是通过对索引上的==索引项==加锁来实现的，而不是对记录加的锁。

1. 行锁(Record Lock):锁定单个行记录的锁，防止其他事务对此行进行update和delete。在RC、RR隔离级别下都支持。
2. 间隙锁(GapLock):锁定索引记录间隙(不含该记录)，确保索引记录间隙不变，防止其他事务在这个间隙进行insert，产生**幻读**。在RR隔离级别下都支持。
...
1. 针对==唯一==索引进行检索时（检索是指where,实际可以是insert...），对==已存在==的记录进行等值匹配时，将会自动优化为行锁。
   - ==主键索引是唯一索引==
...
1. 索引上的等值查询(唯一索引)，给==不存在==的记录加锁时,优化为间隙锁 。
   - 会为该记录后存在的第一条记录==前==加上间隙锁
2. 索引上的等值查询(普通索引)，向右遍历时最后一个值不满足查询需求时，next-keylock退化为间隙锁。
   - 会在符合查询需求的记录之前加上临建锁，在该记录后的间隙加上间隙锁，防止幻读 
   - (n,m],(m,m后的第一个记录]-->n是满足条件的值
   - select...lock in share mode
3. 索引上的范围查询(唯一索引)--会访问到不满足条件的第一个值为止。
   - 会为不满足条件的第一个记录m,前后加上间隙锁 (n,m] ,(m,正无穷]--->n是满足条件的值
   - select...lock in share mode
...
![](assets/21810694716.png)

#### 磁盘结构

![image-20251026165536345](assets/image-20251026165536345.png)

![文件无法预览。](assets/71896219103.png)

![](assets/6345845897.png)

#### 后台线程

![](assets/1192969375.png)
...
- 一致性(Consistency) ：事务完成时，必须使所有的数据都保持一致状态。--undo log+redo log
- 隔离性(lsolation)：数据库系统提供的隔离机制，保证事务在不受外部并发操作影响的独立环境下运行。--锁+mvcc
- 持久性(Durability)：事务一旦提交或回滚，它对数据库中的数据的改变就是永久的。--redo log

特性原理分类图：

![](assets/10723932987.png)

#### redo log

重做日志，记录的是事务提交时数据页的==物理修改==，是用来实现事务的**持久性**。

该日志文件由两部分组成:重做日志缓冲(redo log buffer)以及重做日志文件(redo log file),前者是在内存中，后者在磁盘中。当事务提交之后会把所有修改信息都存到该日志文件中,用于在==刷新脏页到磁盘==**发生错误**时,进行数据恢复使用。

Buffer Pool在产生脏页数据的时候，会先将数据存储到 redo log buffer 再存储到 redo log 中进行磁盘持久化存储，在内存出现异常（比如突然断电）时，通过redo log中持久化的数据进行回滚。过程如下图：

![](assets/7521579557.png)

redo log 要写到磁盘，数据也要写磁盘，为什么要多此一举?

写入 redo log 的方式使用了追加操作，所以磁盘操作是**顺序写**，而写入数据需要先找到写入位置，然后才写到磁盘，所以磁盘操作是**随机写**。

当数据顺利刷新到磁盘后，日志就失去了作用，隔一段时间会清空。-->重做日志是两个文件循环写

#### undo log

回滚日志，用于记录数据被==修改前的信息==，作用包含两个:提供回滚 和 MVCC(多版本并发控制)。
...
读取的是记录的==最新==版本，读取时还要保证其他并发事务不能修改当前记录，会对读取的记录进行加锁。对于我们日常的操作，如:select...lock in share mode(共享锁)，select... for update、update、insert、delete(排他锁)都是一种当前读。

多个事务同时开启时，Repeatable Read会保证事务读取的数据始终与开启事务时的数据相同，不会读取到其他事物提交的数据，但当前读可以读取到其他事物提交的数据（记录的最新版本）
...
- Repeatable Read:开启事务后第一个select语句才是快照读的地方。-->之后的select会直接查询快照
...
![](assets/84711022753.png)

![](assets/6934998817.png)

==依次==（由新到旧）比较 undo log 日志中版本数据链，找到可以进行访问的版本数据。
...
![](assets/3233394951.png)

#### mysqladmin

![](assets/10103131010198.png)

#### mysqlbinlog

![](assets/1285451886.png)

#### mysqlshow

![](assets/8210310352105.png)

#### mysqldump

![](assets/479211610103.png)

#### mysqlimport/source

![](assets/34108898837.png)
...
## 主从复制
 ...
 <img src="assets/58131011726.png" style="zoom:50%;" />

![](assets/4454682776.png)

增删改访问主库，查询访问从库

### 原理

![](assets/7410105910882.png)

### 搭建实现

#### 主库配置

![](assets/21103533211.png)

![](assets/24794193710.png)

![](assets/91294866110.png)

#### 从库配置

![](assets/10617439921.png)

![](assets/21053341798.png)

![](assets/6153782135.png)
...
![](assets/4765397657.png)

![](assets/35239910325.png)

![](assets/106169108266.png)

![](assets/4616215143.png)

水平拆数据，垂直拆结构

![](assets/86213810658.png)

### Mycat概述

![](assets/81010325101039.png)

![](assets/6382138721.png)

![](assets/101795210673.png)

### Mycat入门

![](assets/96457210797.png)

![](assets/1891042610310.png)

![](assets/qwe1324t14t.png)

![](assets/10197188896.png)

![](assets/59710648531.png)
...
![](assets/32368131027.png)

<img src="assets/image-20251027212924391.png" alt="image-20251027212924391" style="zoom:67%;" />

![](assets/7484479134.png)

![](assets/10769359339.png)

![](assets/91315810623.png)

#### rule.xml

![](assets/951010415971.png)

![](assets/84271093639.png)

![](assets/3853211256.png)

### Mycat分片

#### 垂直分库

![](assets/9325751819.png)

![](assets/21931016227.png)

![](assets/9852726692.png)

全局表

![](assets/58109378671.png)

#### 水平分表

![](assets/1269361453.png)

![](assets/9977715254.png)

![](assets/131081045236.png)

#### 分片规则

##### 范围分片

![](assets/34510781883.png)

![](assets/2654319698.png)

##### 取模分片

![](assets/22173110191.png)

![](assets/7347361194.png)

##### 一致性hash算法

![](assets/73610917872.png)

![](assets/3171425417.png)

##### 枚举分片

![](assets/667103121103.png)

![](assets/11088699372.png)

默认节点：指定插入数据超出枚举范围时，会将数据存到哪个节点

##### 应用指定算法

![](assets/2632725279.png)

![](assets/1452119317.png)

##### 固定hash算法

![](assets/2375113118.png)

![](assets/1368697562.png)

##### 字符串hash解析

![](assets/53710618932.png)

![](assets/8619959324.png)

##### 按天分片

![](assets/1010769110977.png)

![](assets/109109981999.png)

##### 按自然月分片

![](assets/4762746916.png)

![](assets/22673141012.png)

### Mycat管理及监控

![](assets/73451065142.png)

![](assets/7348156477.png)

## 读写分离

<img src="assets/52510452433.png" style="zoom:67%;" />

### 介绍

![](assets/10918829811.png)

### 一主一从读写分离

![](assets/52287271210.png)

![](assets/4564143383.png)
<!--  -->

### 双主双从

![](assets/245101066682.png)

![](assets/6338368756.png)

![](assets/2677654314.png)
<!-- -->
![](assets/105101448841.png)

![](assets/7237222812.png)

![](assets/6246592973.png)

![](assets/12919109752.png)
...
![](assets/8536419478.png)

![](assets/15610682864.png)
...
1. 将表中的属性修改为枚举类型 change,modify

```sql
-- 改属性名
ALTER TABLE 表名
CHANGE 旧列名 新列名 ENUM('值1', '值2', '值3', ...) [其他约束];

-- 不改属性名
ALTER TABLE 表名
MODIFY COLUMN 列名 ENUM('值1', '值2', '值3', ...) [其他约束];
```

2. 确保表名和列名没有包含数字和特殊字符，或者如果确实需要包含，确保它们被正确引用。在MySQL中，如果标识符（如表名或列名）包含特殊字符或数字，应该使用==反引号==（`）将其括起来。-->不是单引号
3. 添加约束

| 约束类型       | 语法                                                         |
| -------------- | ------------------------------------------------------------ |
| 主键           | `ALTER TABLE tbl ADD PRIMARY KEY (col);`                     |
| 唯一           | `ALTER TABLE tbl ADD [CONSTRAINT uq_name] UNIQUE (col);`     |
| 非空           | `ALTER TABLE tbl MODIFY col datatype NOT NULL;`              |
| 默认值         | `ALTER TABLE tbl ALTER col SET DEFAULT val;`                 |
| 外键           | `ALTER TABLE tbl ADD CONSTRAINT fk_name FOREIGN KEY (col) REFERENCES parent(col);` |
| CHECK(≥8.0.16) | `ALTER TABLE tbl ADD CONSTRAINT ck_name CHECK (col ≥ 0);`    |

- MySQL 的 DEFAULT 只能是==字面量==，不能是==子查询==、不能引用父表，也不能用函数
- 建立唯一约束时，建表语句会将其当做唯一索引，单起一列

4. MySQL 的 `ALTER TABLE` 语句**一次只能针对一张表**，不允许把多张表用逗号拼在一起批量加列。

```sql
-- 错误
alter table `e3m2ployees`,`m3e2mbers` add p3h2one char(11);
```

5. 索引名不能用单引号 `'...'` —— 单引号是字符串字面量，**标识符（索引名）必须用反引号** ``...`` 或者干脆不用引号。