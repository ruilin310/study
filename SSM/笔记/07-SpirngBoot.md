...
* ==`---` 是用来分隔不同文档的，但它并不是用来明确表示文档的结束，而是用来开始一个新的文档==
...
`properties` 类型的配置文件配置多环境需要定义==不同的配置文件==
...
==classpath是指项目内的resources目录，file是指jar包所在的目录==
...
- 启动 Spring Boot 应用程序上下文
  `@SpringBootTest` 会启动一个完整的 Spring Boot 应用程序上下文，就像运行主应用程序一样。它会加载 `application.properties` 或 `application.yml` 配置文件，初始化所有的 Spring Bean，包括控制器、服务层、数据访问层等。这使得测试类可以像在真实运行环境中一样访问所有的 Spring 组件
