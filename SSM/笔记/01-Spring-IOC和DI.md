...
* 哪些bean对象适合交给容器进行管理?
  ...
  * 数据层对象
  * 工具对象
* 哪些bean对象==不适合==交给容器进行管理?
  * 封装**实例**的域对象，因为会引发线程安全问题，所以不适合。
...
* 错误信息从下往上依次查看，因为==上面的错误大都是对下面错误的一个包装，最核心错误是在最下面==
...
- 方法二:getObjectType(),被重写后，主要返回的是==被创建类的Class对象==-->==返回编译类型（接口）？==

- 方法三:没有被重写，因为它已经给了默认值，从方法名中可以看出其作用是设置对象是否为单例，默认true

从意思上来看，我们猜想默认应该是单例，如何来验证呢?
...
从结果中可以看出，init方法执行了，但是==destroy方法却未执行==，这是为什么呢?
...
- 都不用，web应用，和关闭在tomcat完成

分析上面的实现过程，会发现添加初始化和销毁方法，即需要编码也需要配置，实现起来步骤比较多也比较乱。

Spring提供了两个接口来完成生命周期的控制，好处是可以**不用再进行配置`init-method`和`destroy-method`**
...
> 1.声明userDao属性
>
> 2.生成一个==带有bookDao和userDao参数==的构造函数
>
...
* 按照名称注入中的名称指的是什么?
  ...
  <img src="assets/1629806856156.png" alt="1629806856156" style="zoom: 80%;" />
...
2. 使用按类型装配时（byType）必须保障容器中相同类型的bean==唯一==，推荐使用
3. 使用按名称装配时（byName）必须保障容器中具有指定名称的bean，因变量名与配置耦合，不推荐使用
4. 自动装配**优先级低于setter注入与构造器注入**，同时出现时**自动装配配置失效**
